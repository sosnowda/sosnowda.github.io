...
## 0. ДОСТУП К РЕПОЗИТОРИЮ И ЗАВЕРШЕНИЕ ИТЕРАЦИИ — ОБЯЗАТЕЛЬНЫЙ ПРОТОКОЛ (приказ владельца, 66.19)

**После старта КАЖДОЙ новой итерации задания:**

1. **Сразу после старта итерации** проверь состояние ПОЛНОГО доступа (чтение/запись) и наличие ключа/токена к репозиторию сайта `https://github.com/sosnowda/sosnowda.github.io.git`:
   - токен (fine-grained PAT) передан владельцем в текущей сессии и не пуст;
   - клон/`git fetch origin` работает (чтение), токен имеет право `contents: write` (запись).
   - Норма для fine-grained PAT: `GET /user` может отвечать 403 «Resource not accessible» — это НЕ признак отсутствия доступа; право записи подтверждается успешным push.
2. **Если полного доступа нет** (токен отсутствует/просрочен/только чтение) — **НЕМЕДЛЕННО ОСТАНОВИ текущую итерацию** и потребуй у пользователя предоставить доступ. Правки кода до подтверждённого доступа не делать.
3. **Сразу по окончании итерации ВСЕГДА** (это часть определения готовности патча, как и зелёные тесты):
   - запушить ВСЕ изменения в `main` (пуш = деплой);
   - обновить ворклог В РЕПОЗИТОРИИ: секция раунда в `worklog.md` (+ `CHANGES.md` при видимых изменениях).

**Токен — только в переменной сессии** (`export GH_TOKEN=…`); `git config --global credential.helper store` ЗАПРЕЩЁН (plaintext токен на диск). Пуш — с per-command helper, export токена и push в ОДНОМ bash-вызове, перед push проверять `if [ -z "$GH_TOKEN" ]`.
...
1a. **Видео — обычные файлы git, LFS НЕ используется (раунд 66.10, откат по приказу владельца)**: из-за проблем с файлами Git LFS удалён НАСОВСЕМ — `.gitattributes` снесён, `assets/video/*.mp4|webm` (promo/intro, ~12,3 МБ) снова обычные blob'ы, локальный LFS-конфиг git снят. История НЕ переписывалась: в старых коммитах (66.9-LFS) остались LFS-указатели — они безвредны, текущий HEAD и новые клоны тянут обычные файлы, git-lfs в среде НЕ нужен.
