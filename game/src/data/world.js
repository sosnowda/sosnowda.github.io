...
export function buildMap() {
    ...
    // === РАУНД 63 (п.1 приказа): ОВЧАРНЯ УДАЛЕНА — на её месте (22–24 ×
    // 11–13) стоит НОВЫЙ ЖИЛОЙ ДОМ (Дом Степана, см. BUILDINGS в interiors.js).
    // Овцы и загон убраны; стадо по-прежнему живёт на выпасе за околицей.
    ...
    // Раунд 63: дерево [25,9] снято — на колонке 25 теперь линия частокола
    // у ворот (ряды 3-4 и 6-9, рисует VillageScene).
    // Раунд 64 (п.3): дерево [24,4] снято — стояло В ПРОЁМЕ новой воротни-арки.
    const trees = [
        [7, 4], [15, 4],
        [17, 9], [11, 14], [25, 13],
    ];
...
// === ХОЗЯЙСТВЕННЫЕ ПОСТРОЙКИ И ДЕТАЛИ ДВОРОВ ===
// РАУНД 64 (п.7 приказа владельца): «УДАЛИТЬ ОГОРОДЫ И ПРОЧИЕ ПОСТРОЙКИ
// В ДЕРЕВНЕ, КРОМЕ ДОМОВ И ВОРОТ» — стога, поленница и телега удалены;
// список пуст (рига/овин/баня удалены ещё в раундах 37–39, овчарня — в 63).
// Остались только игровые объекты: колодец ('W'), костёр ('F') и крест ('X')
// — это механики (сбор у колодца, отдых, молитва), а не постройки.
