...
// Раунд 51 (п.11 заявки): карта расширена на ВОСТОК (26→32) — восточная
// слобода: рыночный ряд из трёх лавок и два деревянных дома (дровосек,
// сапожник). Все прежние координаты (дома, NPC, сундуки) не сдвигаются.
...
export function buildMap() {
    ...
    //    Раунд 51: южная лента продлена до кол 28 — двери дома сапожника
    //    (восточная слобода) выходят прямо на грунт.
    for (let x = 2; x <= 28; x++) {
        set(x, 2, 'S');
        set(x, 14, 'S');
    }
    ...
    //    Раунд 51: тянется до новых ворот (кол 31) через восточную слободу.
    ...
    //    Раунд 51: продлена до кол 30 — у избы дровосека (дверь 27,17).
    for (let x = 1; x <= 30; x++) {
        grid[18][x] = 'B';
        grid[19][x] = 'B';
    }
    // Соединение второй улицы с переулком 23 и краем карты
    for (let x = 22; x <= 30; x++) set(x, 18, 'B');
