...
//
// Механика:
// - Вор бежит из деревни в случайном направлении и проходит ПО ДВУМ локациям
...
//   При каждом тике вор перемещается или ждёт на локации. По истечении
//   времени он покидает локацию и идёт в следующую. После второй локации
//   вор сбегает — игра проиграна.
// - Селяне с некоторым шансом (проверка разговорного навыка) дают наводку
//   на первую локацию, в которую переместился вор.
 ...
 * Вор выбирает маршрут из двух случайных локаций и уже бежит из деревни.
 */
export function initThiefHunt(registry) {
    const quest = registry.get('quest') || {};

    // Маршрут: две РАЗНЫЕ локации в случайном порядке
    const pool = [...CHASE_LOCATIONS];
    const firstId = pool.splice(Math.floor(Math.random() * pool.length), 1)[0];
    const secondId = pool[Math.floor(Math.random() * pool.length)];

    quest.chase = {
        route: [firstId, secondId],
        stage: 'to_stop0',     // to_stop0 → at_stop0 → to_stop1 → at_stop1 → (побег)
        ticksLeft: START_TRAVEL_TICKS,
        stay0: 5 + Math.floor(Math.random() * 3), // 5..7 тиков на первой локации
        stay1: 3 + Math.floor(Math.random() * 3), // 3..5 тиков на второй
...
export function isThiefAt(registry, locationId) {
    ...
    if (!c) return false;
    if (c.stage === 'at_stop0') return c.route[0] === locationId;
    if (c.stage === 'at_stop1') return c.route[1] === locationId;
    return false;
}
...
export function chaseTicksLeft(registry) {
    ...
    const c = q.chase;
    switch (c.stage) {
        case 'to_stop0': return c.ticksLeft + c.stay0 + TRAVEL_TICKS + c.stay1;
        case 'at_stop0': return c.ticksLeft + TRAVEL_TICKS + c.stay1;
        case 'to_stop1': return c.ticksLeft + c.stay1;
        case 'at_stop1': return c.ticksLeft;
        default: return 0;
    }
}

/** Где вор сейчас/куда направляется — для подсказок селян. */
function thiefWhereabouts(registry) {
    const c = getChase(registry);
    if (!c) return null;
    switch (c.stage) {
        case 'to_stop0': return { locId: c.route[0], heading: true };
        case 'at_stop0': return { locId: c.route[0], heading: false };
        case 'to_stop1': return { locId: c.route[1], heading: true };
        case 'at_stop1': return { locId: c.route[1], heading: false };
        default: return null;
    }
}
...
function thiefStep(registry, c) {
    ...
    switch (c.stage) {
        case 'to_stop0':
            // Вор добрался до первой локации
            c.stage = 'at_stop0';
            c.ticksLeft = c.stay0;
            break;
        case 'at_stop0': {
            // Вор ушёл с первой локации — оставил следы в сторону второй
            c.stage = 'to_stop1';
            c.ticksLeft = TRAVEL_TICKS;
            c.traces[c.route[0]] = { wentTo: c.route[1] };
            const from = getLocationById(c.route[0]);
            ActionLog.add(registry, tf(t('Вор покинул «{0}» и двинулся дальше.'), from ? from.name : c.route[0]));
            break;
        }
        case 'to_stop1':
            // Вор добрался до второй локации
            c.stage = 'at_stop1';
            c.ticksLeft = c.stay1;
            break;
        case 'at_stop1':
            // После посещения второй локации вор сбегает — проигрыш
...
export function escapeThief(registry) {
    ...
    if (!q || !q.chase) return;
    const c = q.chase;
    c.traces[c.route[1]] = { wentTo: null };
    q.thiefEscaped = true;
    q.currentObjective = t('Вор скрылся с иконой. Погоня провалена.');
    ActionLog.add(registry, t('ПОРАЖЕНИЕ: вор покинул вторую локацию и скрылся из вида. След ведёт за околицу.'));
    registry.set('quest', q);
}
 ...
 * - Вор здесь сейчас → «свежайшие следы» (его и так видно).
 * - Вор был здесь и ушёл → успех spot: следы читаются, видно направление.
 * - Вора здесь не было → следов нет (локация помечается обысканной).
 * При провале spot следы можно искать повторно (каждая попытка — тик).
 */
export function searchLocation(registry, locationId) {
    ...
    const spotSkill = (player.skills && player.skills.spot) || 25;
    ...
    if (trace) {
        ...
        if (res.result === 'critical' || res.result === 'success') {
            ...
            if (trace.wentTo) {
                ...
                message = tf(t('Вор был здесь! Следы ведут в сторону «{0}». Не теряй времени!'), next ? next.name : trace.wentTo);
            } else {
        ...
        return {
            found: false, alreadySearched: false,
            message: t('Кто-то здесь проходил — видны примятые травы. Но разобрать следы не вышло. Попробуй ещё раз.'),
            turnsLeft: chaseTicksLeft(registry), thiefEscaped: false,
        };
    }
...
export function askNPC(registry, npcId, npcName) {
    ...
    const player = registry.get('player');
    const oratorySkill = (player.skills && player.skills.oratory) || 15;
...
export function askMoneyForHelp(registry, npcId, npcName) {
    ...
    const player = registry.get('player');
    const persuadeSkill = (player.skills && player.skills.persuade) || 20;
...
export function stunThief(registry) {
    ...
    const player = registry.get('player');
    const brawlSkill = (player.skills && player.skills.brawl) || 25;
...
/** Вор в панике бежит немедленно. С первой локации — в следующую, со второй — прочь. */
function thiefFleesNow(registry) {
    ...
    // at_stop0 → немедленно в путь ко второй локации
    c.stage = 'to_stop1';
    c.ticksLeft = TRAVEL_TICKS;
    c.traces[c.route[0]] = { wentTo: c.route[1] };
    registry.set('quest', q);
