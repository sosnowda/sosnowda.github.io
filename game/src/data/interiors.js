...
export const BUILDINGS = [
    // РАУНД 66.25 (приказ 2): «ПЕРЕМЕСТИТЬ ИЗ ПРАВОГО ВЕРХНЕГО УГЛА ЛАВКУ —
    // ЕЁ ПЕРЕКРЫВАЕТ КАРТА ЛОКАЦИИ». Под виджетом «План деревни» (правый
    // верхний угол экрана) стояла КУЗНИЦА с кузнечным двором-«лавком» — 75%
    // здания было скрыто картой. Планировка ПЕРЕСОБРАНА ротацией пяти домов:
    // кузница ← дом ремесленника ← знахарка ← мясник ← Прасковья (юг);
    // северо-восточный угол теперь ПУСТ (трава и дерево) — план деревни
    // больше ничего не перекрывает. Зазоры между домами ≥1 тайла сохранены.
    ...
    // Дом ремесленника — из северо-восточного края в северный ряд
    { interiorId: 'shop_tools', col: 15, row: 1, w: 2, h: 3, label: t('Дом ремесленника') },
    // Кузница — ПЕРЕМЕЩЕНА с северо-восточного края (п.2 66.25: была под
    // картой локации); каменный фасад разделяет два одинаковых фахверка
    { interiorId: 'blacksmith', col: 18, row: 1, w: 2, h: 3, label: t('Кузница') },
    // Знахарка — из среднего ряда в северный (её старое место — мяснику)
    { interiorId: 'healer_house', col: 21, row: 1, w: 2, h: 3, label: t('Дом знахарки') },
    // Северо-восточный угол (колонки 23–24) — ПУСТ: под планом деревни только
    // трава и дерево (66.25, п.2)
    ...
    // Мясник Потап — из южного ряда в средний (место знахарки)
    { interiorId: 'butcher_house', col: 23, row: 6, w: 2, h: 3, label: t('Дом Потапа') },
    ...
    // Прасковья — из северного ряда в южный (широкий дом у восточного частокола)
    { interiorId: 'grocer_house', col: 22, row: 10, w: 3, h: 3, label: t('Дом Прасковьи') },
];
