...
// Раунд 66.24 (приказ 2 владельца): ТРАКТ РАЗДЕЛЁН НА ДВЕ ЛОКАЦИИ —
// Южный Тракт (упирается в Реку и идёт через мост через реку) и
// Северный Тракт. Оба по виду почти одинаковые, у каждого стоит
// указатель направления на деревню. id road_south сохранён (сейвы,
// наводки, маршруты вора), road_north — новый.
...
export const MAP_LOCATIONS = [
    ...
    {
        // Раунд 66.24: бывший «Тракт на юг» — теперь ЯВНО Южный: от ворот
        // деревни на юг, упирается в Реку и проходит через мост.
        id: 'road_south',
        name: t('Южный Тракт'),
        icon: '🛤',
        description: t('Большой тракт на юг: от деревни до самой реки, через мост на другой берег. По нему ходят купеческие обозы.'),
        type: 'road',
        danger: 'medium',
        canFight: true,
        enemies: ['bandit'],
    ...
    {
        // Раунд 66.24 (приказ 2): Северный Тракт — новая локация, зеркальная
        // южной: от ворот деревни на север, к северным волостям.
        id: 'road_north',
        name: t('Северный Тракт'),
        icon: '🛤',
        description: t('Большой тракт на север: уходит от деревни за горизонт, к северным волостям. По нему гонят скот и возят рыбу.'),
        type: 'road',
        danger: 'medium',
        canFight: true,
        enemies: ['bandit'],
...
// Раунд 66.24: road_north добавлен к списку развилки (10 кнопок).
export function getForkLocations() {
    return MAP_LOCATIONS.filter(l =>
        ['forest_edge', 'road_south', 'road_north', 'field', 'lake', 'pogost', 'mill', 'apiary', 'pasture', 'river'].includes(l.id)
    );
}
