...
export class VillageScene extends Phaser.Scene {
    ...
    /**
     * РАУНД 66.25 (приказ 3): воротня НОВОГО ПОКОЛЕНИЯ — «створ поперёк дороги».
     * Прежняя village_gate_r66 (профиль r65) выглядела сломанной: узкая башня
     * стояла ПРЯМО на проезде, второй столб читался случайной палаткой, вместо
     * створок — чёрный блоб. Теперь ворота стоят ПОПЕРЁК дороги (створ север-юг):
     *  - СЕВЕРНАЯ башня (village_gate_r67_north, 56×96) над дорогой, в стиле
     *    частокола (частокол подходит к воротам с севера и юга) — ДАЛЬНИЙ слой,
     *    глубина 4.45 (игрок на дороге рисуется ПЕРЕД ней);
     *  - ЮЖНАЯ группа (village_gate_r67_south, 96×148): дубовая поперечина над
     *    проездом с подвесным ФОНАРЁМ (тёплое стекло), створки РАСКРЫТЫ к
     *    деревне (запад), южная башня с каменным основанием — БЛИЖНИЙ слой,
     *    глубина 7.45 (игрок проходит СКВОЗЬ ворота ПОД фонарём и ЗА створками);
     *  - проезд (ряд ворот 'G') полностью прозрачен — дорога видна насквозь.
     * Геометрия: линия ворот смещена к востоку — башни x 1214..1256 (центр
     * gx+12 = 1236); дорога — ряд VILLAGE_GATE.row (y 240..288);
     * север: x 1212..1260, y 144..240; юг: x 1176..1272, y 216..364
     * (текстура шире — створки уходят на запад к деревне).
     */
    drawVillageGate(ts) {
        const gx = MAP_W * ts - ts / 2;
        // ЛИНИЯ ВОРОТ смещена к восточному краю (башни x 1214..1256, центр
        // 1236 = gx+12) — дом мясника (23,6) не касается башен.
        if (this.textures.exists('village_gate_r67_north')) {
            const north = this.add.image(gx + 12, VILLAGE_GATE.row * ts, 'village_gate_r67_north');
            north.setOrigin(0.5, 1);        // якорь: низ по центру, на кромке дороги
            north.setDepth(VILLAGE_GATE.row - 0.55);   // 4.45 — дальний слой
        }
        if (this.textures.exists('village_gate_r67_south')) {
            // Текстура 96×148 охватывает мир x (gx−48)..(gx+48), y
            // (row*ts − 24)..(row*ts + 124): верхние 72px — поперечина/фонарь/
            // створки НАД дорогой, нижние — южная башня.
            const south = this.add.image(gx, VILLAGE_GATE.row * ts - 24, 'village_gate_r67_south');
            south.setOrigin(0.5, 0);        // якорь: верх по центру
            south.setDepth(VILLAGE_GATE.row + 2 + 0.45); // 7.45 — ближний слой
            // Мягкое тёплое свечение фонаря (стекло фонаря: мир (1230, 248))
            if (this.textures.exists('glow_soft')) {
                const lantern = this.add.image(gx + 6, VILLAGE_GATE.row * ts + 8, 'glow_soft')
                    .setDisplaySize(44, 44)
                    .setTint(0xffb54a)
                    .setBlendMode(Phaser.BlendModes.ADD)
                    .setAlpha(0.32)
                    .setDepth(VILLAGE_GATE.row + 2 + 0.44);
                lantern.__k = 0.18;
                lantern.__flicker = true;
                lantern.__fs = 0.6 + Math.random() * 0.6;
                lantern.__ph = Math.random() * Math.PI * 2;
                this.windowGlows.push(lantern);
            }
        }
    }
     ...
     * РАУНД 66.21 (приказы 9–11): ЖИЛЫЕ ДОМА ЗАПИРАЮТСЯ НА НОЧЬ, даже если
     * хозяин внутри спит; ЦЕРКОВЬ открыта на богослужение и 2–3 часа после
     * (systems/AccessHours.js), ночью и в час отдыха — заперта.
     * РАУНД 66.22 (приказы 1–2): ночь — ПО ВРЕМЕНИ ГОДА И ДАТЕ: дома
     * открываются С РАССВЕТОМ и запираются ПО ЗАКАТУ (sunTimes: широта
     * Новгородской земли ≈ 58.5° с.ш.); летом двери заперты ~21:15–02:50,
     * зимой ~15:10–08:50. Открыты всегда: постоялый двор (public: true).
    ...
    getInteriorClosure(interiorId) {
        ...
        // Раунд 66.22: ночь считается по закату/рассвету текущей даты
        if (isNightHour(hour, timeState)) return { night: true };
     ...
     * жильцов это злит (−1 к личной репутации), но при срочном деле
     ...
     * Жильцов это злит (−1 к личной репутации отвечающего, NightKnock.js),
