...
console.log('— п.2: ДОСКА ПОРУЧЕНИЙ У ВОРОТ —');
{
    ...
    // уникальные награды не раздаются с доски
    ok(vs.includes('uniqueFromElder') && vs.includes('elderSwordPromised = false'),
        'меч старосты с доски не уходит (обещание откатывается)');
}
...
{
    const sw = read('sw.js');
    ok(sw.includes("var CACHE_NAME = 'chronicles-ruthenia-v66'"), 'SW: сайт v66');
