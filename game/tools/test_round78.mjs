...
console.log('— приказы 1–3 во всех точках еды —');
{
    ...
    // РАУНД 66.17 (уточнение п.1): простая еда +1, ПОЛНОЦЕННАЯ в трактире 2–3 HP
    ok(/id: 'bread'[\s\S]{0,90}heal: 2/.test(isr), 'лавка: хлеб = полноценная еда (+2 HP, 66.17)');
    ok(/id: 'kasha'[\s\S]{0,90}heal: 3/.test(isr), 'лавка: каша = полноценная еда (+3 HP, 66.17)');
    ok(/id: 'mead'[\s\S]{0,90}heal: 1/.test(isr), 'лавка: медовуха = питьё (+1 HP)');
    ok(/id: 'kvass'[\s\S]{0,90}heal: 1/.test(isr), 'лавка: квас = питьё (+1 HP)');
    ...
    // РАУНД 66.17 (п.7): рыбалка больше НЕ еда — улов (сырая рыба) в узел;
    // сырую рыбу нельзя есть, есть кулдауна еды рыбалка не касается
    ok(loc.includes("countOf(player, 'rod') > 0"), '66.17: рыбалка требует удочку/чужую уду');
    ok(loc.includes("addItem(player, 'fish_raw', catchN)"), '66.17: улов — сырая рыба в узел');
    ok(!loc.includes('canEat') && !loc.includes('registerMeal'), '66.17: рыбалка вне правил еды (без кулдауна)');
    ok(loc.includes('fishingCatchCount'), '66.17: улов по погоде/сезону (fishingCatchCount)');
    ok(loc.includes('const raining = weather && isRainy(weather);'), 'рыбалка: погода осталась в подсчёте улова');
}
...
{
    const isr = read('game/src/scenes/InteriorScene.js');
    ok(isr.includes('[2, 3, 4, 6, 8, 12]'), '66.17: варианты сна 2..12 часов (минимум 2 ч)');
    ...
    ok(isr.includes('if (pct >= 1) {'), '66.17: 8+ ч — полное, меньше — пропорционально (restHealPct)');
    ok(isr.includes('restHealPct(sleepMinutes)'), '66.17: лечение — по доле сна (8 ч = 100%)');
}
...
console.log('— приказ 6: женский портрет воровки —');
{
    ...
    for (const orphan of ['Отдохнуть 1 час (4 д.) — лечение ~1/3', 'Ночлег 8 часов (12 д.) — полное восстановление',
        ...
        '— сон {0} ({1} д.)', '— лечение ~1/3', 'Свежая рыба', 'Отдохнуть у костра (1 час)']) {
    ...
    for (const k of ['🥣 Герой сыт', '😴 Герой не хочет спать', 'Герой не хочет спать — он только недавно встал. Отдых отменён.',
        ...
        '— сон {0} ({1} д.), ~{2}% здоровья', 'Ночлег {0} ч ({1} д.) {2}', 'час времени']) {
